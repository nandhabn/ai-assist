...
- **New State**: A `calculateSimilarity` function now performs token-based fuzzy matching. It normalizes strings and calculates a Jaccard similarity score. The system now picks the on-page element that is _most semantically similar_ to the AI's suggestion, with a configurable threshold (`> 0.5`) to discard poor matches.
